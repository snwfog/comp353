...
class Database
{
    ...
    public function query($sql)
    {
        ...
        $this->mysqli_raw = $this->mysqli->query($sql);

        if (!$this->mysqli_raw && $this->debug)
        {
     ...
     * $db->selectField($name);
     *
     * @param null $select (Default is SELECT *)
     * @param int $fetchMode
     * @return array
     */
    public function selectField($select = NULL, $fetchMode = MYSQL_BOTH)
    {
        if ($select)
        {
            $row = $this->mysqli_raw->fetch_array($fetchMode);
            $data = $row[$select];
        }
        else
        {
            ...
            while ($row = $this->mysqli_raw->fetch_array($fetchMode))
                $data[] = $row;
        }

        $this->mysqli_raw->close();
